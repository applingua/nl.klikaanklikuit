...
module.exports = RFDevice => class Kaku extends RFDevice {
	...
	static codewheelsToData(codewheelIndexes) {
		if (codewheelIndexes.length === 2) {
			...
			const data = {
				address: Math.floor(codewheelIndexes[0] / 3).toString(2).padStart(4, 0),
				channel: '00',
				unit: Math.floor(codewheelIndexes[0] % 3).toString(2).padStart(4, 0),
				undef: [0, 1, 1],
				state: 0,
			};
			data.id = `${data.address}:${data.channel}:${data.unit}`;
