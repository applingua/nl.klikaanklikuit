...
const { dataToPayload: dataToPayloadOld } = require('../kakuold/kaku');
...
const dataToPayload = function(data) {
	...
	if (
		...
		// no kaku signal so check of we should use kakuold as backup
	} else if (
		data &&
		data.address && data.address.length === 4
	) {
		console.log('Using old signal in kaku');
		return dataToPayloadOld(data);
	}
